package q.rest.cart.operation.sockets;


import q.rest.cart.helper.AppConstants;
import q.rest.cart.operation.AsyncService;
...
public class NotificationsEndPoint {
    ...
    public void onOpen(Session session, @PathParam("username") Integer userId, @PathParam("token") String token) throws IOException {
        System.out.println("Connected to cart ws");
        this.session = session;
        this.userId = userId;
        this.token = token;
        if(this.tokenMatched()) {
            notificationsEndPoints.add(this);
            broadcast("wireRequests,"+async.getWireRequestCount());
            broadcast("processCarts," + async.getProcessCartCount());
        }
        else {
            session.close();
        }
